...
float initialY = 0.0;
float initialZ = 0.0;
bool initialAnglesRecorded = false;

// Function to normalize angles to -180 to 180 range
int normalizeAngle(float angle) {
    int normalized = (int)angle % 360;
    if (normalized > 180) {
        normalized -= 360; // Convert to negative range for angles > 180
    } else if (normalized < -180) {
        normalized += 360; // Convert to positive range for angles < -180
    }
    return normalized;
}
...
void loop() {
    ...
    if ((millis() - timer) > 100) {  // send data every 100ms
        ...
        if (!initialAnglesRecorded) {
            initialX = angleX;
            initialY = angleY;
            initialZ = angleZ;
            initialAnglesRecorded = true;
        }

        // Adjust angles relative to initial values and normalize to -180 to 180
        int relativeX = normalizeAngle(angleX - initialX);
        int relativeY = normalizeAngle(angleY - initialY);
        int relativeZ = normalizeAngle(angleZ - initialZ);

        if (isnan(relativeX) || isnan(relativeY) || isnan(relativeZ)) {
            Serial.println("Error: Unable to read angle values from MPU6050. Please check sensor connection and wiring.");
        } else {
            Serial.print(relativeX);
            Serial.print(",");
            Serial.print(relativeY);
            Serial.print(",");
            Serial.println(relativeZ);
        }

        timer = millis();

        if (Firebase.ready()) {
            Firebase.setInt(fbdo, F("/angle/x"), relativeX);
            Firebase.setInt(fbdo, F("/angle/y"), relativeY);
            Firebase.setInt(fbdo, F("/angle/z"), relativeZ);
        }
    }
}
