...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function highlightGridItem(index) {
        ...
        if (index >= 0 && index < gridItems.length) {
            ...
            // Update count or display 'Record' based on active tab
            let countLabel = selectedItem.querySelector('.count-time-label, .record-button');
            if (setDefaultTab.classList.contains('active')) {
                countLabel.textContent = 'Record';
            } else if (gazeTrackerTab.classList.contains('active')) {
    ...
    resetButton.addEventListener('click', () => {
        document.querySelectorAll('.grid-item').forEach((item) => {
            // Reset tracking data
            let countLabel = item.querySelector('.count-time-label, .record-button');
    ...
    aboutTab.addEventListener('click', () => {
        ...
        // Update grid items to show gaze pattern name only
        gridItems.forEach(item => {
            let countLabel = item.querySelector('.count-time-label, .record-button');
            countLabel.textContent = '';
        });
    ...
    setDefaultTab.addEventListener('click', () => {
        ...
        // Update grid items to show 'Record' label
        gridItems.forEach(item => {
            let countLabel = item.querySelector('.count-time-label, .record-button');
            countLabel.textContent = 'Record';
        });
    ...
    gazeTrackerTab.addEventListener('click', () => {
        ...
        // Update grid items to show count and time labels
        gridItems.forEach(item => {
            let countLabel = item.querySelector('.count-time-label, .record-button');
            let countValue = countLabel.getAttribute('data-count') || '0';
            countLabel.textContent = `Count: ${countValue} | Time: 0s`;
        });
    });
        ...
        recordButton?.addEventListener('click', () => {
            gyroDataRef.once('value', (snapshot) => {
                if (snapshot.exists()) {
                    const data = snapshot.val();
                    if (data && typeof data === 'object') {
                        recordButton.textContent = `Recorded: X=${data.x.toFixed(2)}, Y=${data.y.toFixed(2)}, Z=${data.z.toFixed(2)}`;
                    } else {
